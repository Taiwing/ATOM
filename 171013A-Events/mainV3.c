...
int main()
{
  ...
  SDL_ShowCursor(SDL_DISABLE); //on masque le curseur de la souris tant que celui-ci est à l'intérieur de la fenêtre
  SDL_WarpMouseInWindow(window, windowSurface->w / 2, windowSurface->h / 2); //grâce à geordi la Forge, on déplace la souris au centre de la fenêtre
  ...
  while(continuer) //on va utiliser une boucle infinie de type "while" pour gérer les évènements
  {
    ...
    switch(event.type) //on teste le type d'évènements
    {
      case SDL_QUIT: //si l'évènement est de type "Quitter"
        ...
        break;
      case SDL_MOUSEMOTION: //"zozor" va suivre les déplacements de la souris
        pZozor.x = event.motion.x;
        pZozor.y = event.motion.y;
        break;
