...
int main()
{
  ...
  SDL_Delay(5000); //met en pause 5s pour qu'on puisse voir la fenêtre en violet
  ...
  pause(); //met en pause pour qu'on puisse voir les changements (l'affichage de "sprite")

  SDL_FreeSurface(sprite); //on libére la mémoire
  SDL_FreeSurface(pWinSurf); //on libére la mémoire
  SDL_DestroyWindow(pWindow); //on libére la mémoire
