...
void update_mini_batch(neural_net *net, data_set *batch, double l_rate)
{
  ...
  for(int i = 0; i < batch->set_size; i++)
  {
    ...
    for(int j = 0; j < net->num_layers-1; j++)
    {
      for(int m = 0; m < net->sizes[j+1]; m++)
      {
        ...
        /*TEST*/
        /*if(!i)
          printf("n_b[%d][%d] += delta_b[%d][%d] = %.17g + %.17g = %.17g\n", j, m, j, m, n_b[j][m]-delta_b[j][m], delta_b[j][m], n_b[j][m]);*/
      }
      for(int k = 0; k < net->sizes[j]; k++)
        for(int l = 0; l < net->sizes[j+1]; l++)
        {
          ...
          /*TEST*/
          /*if(!i)
            printf("n_w[%d][%d][%d] += delta_b[%d][%d][%d] = %.17g + %.17g = %.17g\n", j, k, l, j, k, l, n_w[j][k][l]-delta_w[j][k][l], delta_w[j][k][l], n_w[j][k][l]);*/
        }
    }
    /*TEST*/
    /*if(!i)
  ...
  for(int i = 0; i < net->num_layers-1; i++)
    for(int j = 0; j < net->sizes[i+1]; j++)
    {
      ...
      /*TEST:*/
      //printf("n_b[%d][%d]/batch->set_size = %.17g / %d = %.17g\n", i, j, n_b[i][j], batch->set_size, (n_b[i][j]/batch->set_size));
    }
  for(int i = 0; i < net->num_layers-1; i++)
    for(int j = 0; j < net->sizes[i]; j++)
      for(int k = 0; k < net->sizes[i+1]; k++)
      {
        ...
        /*TEST:*/
        //printf("n_w[%d][%d][%d]/batch->set_size = %.17g / %d = %.17g\n", i, j, k, n_w[i][j][k], batch->set_size, (n_w[i][j][k]/batch->set_size));
      }
