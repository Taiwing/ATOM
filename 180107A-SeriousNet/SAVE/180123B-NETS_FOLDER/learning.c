...
/*void backprop(neural_net *net, data_set *batch, double *delta_n_b, double ***delta_n_w)
{
  ...
  {
    TO DO: CHANGE THE BIAS ARRAY TO A TWO DIMENSIONNAL ARRAY
    delta_n_b[olay-1][i] = (net->layers[olay][i]->output - batch->expout[i]) * (*get_prime(&logistic))(net->layers[olay][i]->input);
