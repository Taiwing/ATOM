/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   format_data.c                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yforeau <yforeau@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/11/17 13:10:09 by yforeau           #+#    #+#             */
/*   Updated: 2018/11/18 20:24:18 by yforeau          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "format_data.h"
#include "libft.h"

static void	apply_prec(t_pdata *l, t_params *conv, int *size, int fdat[4])
{
	fdat[ZPAD] = conv->prec - *size;
	if (l->buf[0] == '-')
	{
		fdat[SIGN] = '-';
		l->buf[0] = '0';
		(*size)++;
	}
	*size = *size + fdat[ZPAD];
}

static void	apply_hash(t_pdata *l, char t, int *size, int fdat[4])
{
	if (t == 'o' && !fdat[ZPAD] && (l->buf[0] != '0' || !l->n))
	{
		fdat[ZPAD]++;
		(*size)++;
	}
	else if (ft_strchr("xXpP", t) && ft_strcmp("0", l->buf))
	{
		*size = *size + 2;
		fdat[HEX] = t == 'x' || t == 'p' ? 1 : 2;
	}
}

static void	int_format(t_params *conv, t_pdata *l, int fdat[4], int *size)
{
	if (conv->prec > *size - (l->buf[0] == '-'))
		apply_prec(l, conv, size, fdat);
	else if (!conv->prec && *size == 1 && l->buf[0] == '0')
	{
		(*size)--;
		(l->n)--;
		l->buf[0] = 0;
	}
	if (conv->type != 'd' && (conv->flags & F_HASH
		|| conv->type == 'p' || conv->type == 'P'))
		apply_hash(l, conv->type, size, fdat);
	if (ft_strchr("dpP", conv->type) && l->buf[0] != '-'
		&& (conv->flags & F_PLUS || conv->flags & F_SPACE))
	{
		fdat[SIGN] = conv->flags & F_PLUS ? '+' : ' ';
		(*size)++;
	}
}

static void	apply_fw(t_pdata *l, t_params *conv, int fdat[4], int *size)
{
	if (ft_strchr("duoxXpP", conv->type) && *size < conv->fw
		&& conv->flags & F_ZERO && !(conv->flags & F_MINUS) && conv->prec < 0)
	{
		fdat[ZPAD] += conv->fw - *size - (l->buf[0] == '-');
		*size = conv->fw;
		if (l->buf[0] == '-')
		{
			fdat[SIGN] = '-';
			l->buf[0] = '0';
		}
	}
	if (*size < conv->fw)
	{
		fdat[SPAD] = conv->fw - *size;
		fdat[SPAD] *= conv->flags & F_MINUS ? -1 : 1;
		*size = conv->fw;
	}
}

void		format_data(t_pdata *d, t_pdata *l, t_params *conv, int size)
{
	int		fdat[4];

	size = l->n;
	fdat[SPAD] = 0;
	fdat[ZPAD] = 0;
	fdat[SIGN] = 0;
	fdat[HEX] = 0;
	if (ft_strchr("duoxXpP", conv->type))
		int_format(conv, l, fdat, &size);
	apply_fw(l, conv, fdat, &size);
	if (fdat[SPAD] > 0)
		add_to_buf(d, NULL, ' ', fdat[SPAD]);
	if (d->n != -1 && fdat[SIGN])
		add_to_buf(d, NULL, fdat[SIGN], 1);
	if (d->n != -1 && fdat[HEX])
		add_to_buf(d, fdat[HEX] == 1 ? "0x" : "0X", 0, 2);
	if (d->n != -1 && fdat[ZPAD] > 0)
		add_to_buf(d, NULL, '0', fdat[ZPAD]);
	if (d->n != -1)
		add_to_buf(d, l->buf, 0, l->n);
	if (d->n != -1 && fdat[SPAD] < 0)
		add_to_buf(d, NULL, ' ', fdat[SPAD] * -1);
}
