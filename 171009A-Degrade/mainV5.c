...
int main()
{
  ...
  for(int i = 0; i < 512; i++)
  {
    ...
    if(i < 256)
      SDL_FillRect(&lignesDegrade[i], NULL, SDL_MapRGB(lignesDegrade->format, 255, i, i)); //on remplit chaque ligne par un couleur de plus en plus claire
    else
    {
      SDL_FillRect(&lignesDegrade[i], NULL, SDL_MapRGB(lignesDegrade->format, 255, c, c)); //on remplit chaque ligne par un couleur de plus en plus claire
      c--;
    }
