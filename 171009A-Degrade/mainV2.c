...
int main()
{
  ...
  SDL_Window *fDegrade = SDL_CreateWindow("Dégradé", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 640, 256, SDL_WINDOW_SHOWN); //on crée la fenêtre
  ...
  for(int i = 0; i < 256; i++)
  {
    ...
    SDL_FillRect(&lignesDegrade[i], NULL, SDL_MapRGB(lignesDegrade->format, c, c, c)); //on remplit chaque ligne par un couleur de plus en plus claire
