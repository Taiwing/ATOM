...
int main()
{
  ...
  for(int i = 0; i < 512; i++)
  {
    ...
    if(i < 256)
      SDL_FillRect(&lignesDegrade[i], NULL, SDL_MapRGB(lignesDegrade->format, i, i, 255)); //on remplit chaque ligne par un couleur de plus en plus claire
    else
    {
      SDL_FillRect(&lignesDegrade[i], NULL, SDL_MapRGB(lignesDegrade->format, c, c, 255)); //on remplit chaque ligne par un couleur de plus en plus claire
      c--;
    }
