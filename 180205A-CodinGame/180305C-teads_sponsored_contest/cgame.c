...
int main()
{
  ...
  // The minimal amount of steps required to completely propagate the advertisement
    ...
    /*TEST*/
    /*node *ptr = tr->first;
    link *lk = tr->first->links->first;

    printf("\nncount: %d\n", tr->ncount);

    for(int i = 0; i < tr->ncount; i++)
    {
      printf("\nLINKS NODE %d:\n", ptr->ID);
...
int shortest_path(node *root)
{
  ...
  for(int i = 0; i < root->lcount; i++)
    max_val = max[i] > max_val ? max[i] : max_val;

  for(int i = 0; i < root->lcount; i++)
    max_val2 = max[i] > max_val2 && max[i] != max_val ? max[i] : max_val2;
