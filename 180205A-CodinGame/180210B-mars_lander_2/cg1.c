...
{
	acc[0] = f_data[angle] ? (double)f_data[power_d] * cos((90 - (double)abs(f_data[angle])) * (PI/180.0)) : (double)f_data[power_d];
	acc[0] = G - acc[1];
	acc[1] = f_data[angle] ? (double)f_data[power_d] * cos((double)f_data[angle] * (PI/180.0)) : 0;
	acc[1] *= f_data[angle] > 0 ? -1 : 1;
