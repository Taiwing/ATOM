...
int main()
{
	...
	while(1)
	{
		...
		else if(f_data[X] < surface[lz[0]][X]+10) //if the landing_zone in on the right
		{
			ob = obstacle(f_data[X], f_data[Y], surface, lz, 'r');
		...
		else if(f_data[X] > surface[lz[1]][X]-10) //if the landing_zone in on the left
		{
			ob = obstacle(f_data[X], f_data[Y], surface, lz, 'l');
			if(ob == -1) //if the shuttle is above every point between the landing_zone and itself including the landing_zone
			{
				rot = 45;
				power = f_data[hSpeed] < -80 ? 0 : 4;
