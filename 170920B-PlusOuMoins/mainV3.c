...
int main()
{
  ...
  int nbMystere = 0, nbJoueur = 0, nbCoups = 0, continuerPartie = 1, ouiNon = 2, deuxJoueurs = 0, compteur = 0;
  ...
  while(continuerPartie)
  {
    ...
    do
    {
      if(deuxJoueurs)
      {
        ...
        printf("euillez rentrer un nombre entre 1 et 100: ");
        scanf("%d", &nbMystere); //PUTAIN JE L'AI ENCORE OUBLIE CET ENCULE

        if((nbMystere < 1 || nbMystere > 100) && deuxJoueurs)
        {
          printf("ERREUR: choix du nombre mystère invalide\n\n");
        }
      }
    } while((nbMystere < 1 || nbMystere > 100) && deuxJoueurs);
