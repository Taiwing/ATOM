...
int main()
{
  ...
  al_install_mouse(); //on intialise la souris
  ...
  al_register_event_source(event_queue, al_get_mouse_event_source()); //puis à la souris
  ...
  while(1) //tant que 1=1 lol (on fait une boucle infinie, qu'on arrête avec "break")
  {
    ...
    else if(ev.type == ALLEGRO_EVENT_MOUSE_AXES || ev.type == ALLEGRO_EVENT_MOUSE_ENTER_DISPLAY) //si la souris bouge dans la fenêtre
    {
      bouncer_x = ev.mouse.x; //on met le bouncer aux coordonnées de la souris en x
      bouncer_y = ev.mouse.y; //puis en y
    }
    else if(ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) //si l'utilisateur appuie sur un bouton de la souris
    {
      break; //on quitte
    }
