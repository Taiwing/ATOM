...
int main()
{
  ...
  int rouge = 255; //on initie à 255 la valeur du rouge dans l'image
  bool valMin = false; //on crée un bool pour inverser le dégradé quand on atteint le bout
  ...
  while(1) //tant que 1=1 lol (on fait une boucle infinie, qu'on arrête avec "break")
  {
    ...
    if(ev.type == ALLEGRO_EVENT_TIMER) //quand on a un "tick" du timer, on raffiche la fenêtre
    {
      ...
      if(!valMin) //tant que l'écran n'est pas noir
      {
        rouge--; //on retire du rouge
        if(rouge == 0) //si rouge = 0
          valMin = true; //on a atteint la valeur minimale
      }
      else
      {
        rouge++; //on ajoute du rouge
        if(rouge == 255) //si on atteint le maximum
          valMin = false; //alors on repasse à la condition du dessus
      }
    }
    ...
    if(redraw && al_is_event_queue_empty(event_queue)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      al_clear_to_color(al_map_rgb(rouge, 0, 0)); //on colore l'écran en rouge
      al_flip_display(); //on met à jour l'écran
    }
  }
