...
int main()
{
  ...
  int degrade = 0; //on initie à 0 la valeur du dégradé
  bool valMax = false; //on crée un bool pour inverser le dégradé quand on atteint le bout
  ...
  while(1) //tant que 1=1 lol (on fait une boucle infinie, qu'on arrête avec "break")
  {
    ...
    if(redraw && al_is_event_queue_empty(event_queue)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      if(!valMax) //tant que l'écran n'est pas blanc
      {
        degrade++; //on retire du rouge
        if(degrade == 255) //si degrade = 255
          valMax = true; //on a atteint la valeur maximale
      }
      else
      {
        degrade--; //on ajoute du rouge
        if(degrade == 0) //si on atteint le minimum
          valMax = false; //alors on repasse à la condition du dessus
      }
      al_clear_to_color(al_map_rgb(255, degrade, degrade)); //on colore l'écran en rouge
      al_flip_display(); //on met à jour l'écran
    }
  }
