...
packOptions *genererPackOptions(elementsFondOptions *elem, ALLEGRO_BITMAP *fondPrecedent, int keyMap[9][3]); //on génère la structure d'images
...
ALLEGRO_BITMAP *genererBoutonOptions(int typeDeBouton, int onOff, elementsFondOptions *elem, int keyMap[9][3]);
void strTouche(int typeDeBouton, int keyMap[9][3], char str[40]); //va renvoyer un string contenant le nom de la touche correspondant au bouton entré
