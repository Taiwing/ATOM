...
void jouerSon(soundConfig *sound, packSons *sons, File *soundEv)//, int r, bool modifVolume)
{
  if(sound->soundOnOff[SOUND] && sound->soundOnOff[THEME] && (!al_get_sample_instance_playing(sons->theme))) //si le son est activé et que le thème ne joue plus
  {
