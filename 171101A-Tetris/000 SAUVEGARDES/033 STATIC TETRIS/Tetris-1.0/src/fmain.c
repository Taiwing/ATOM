...
void menuPrincipal()
{
  ...
  //on applique la résolution contenue dans le fichier de configuration
  setRes(screen, NULL, NULL, config, true);

  //on génère les images du menu et on l'affiche
  elements *elem = genererElements(); //on crée les structures coord du jeu
  resizeElements(elem, screen->scaleBuff); //on initialise les structures utilisées pour les dessins
  packImages *images = genererPackImages(elem, config->keyMap, screen->scaleBuff); //on génère les images du jeu

  //on affiche le jeu à la résolution indiquée par la configuration
