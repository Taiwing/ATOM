...
packPartie *genererPackPartie(elements *elem, int modeDeJeu, float scale)
{
  ...
  images->blocJaune = genererBloc(JAUNE, scale);
  images->blocCyan = genererBloc(CYAN, scale);
  images->blocMauve = genererBloc(MAUVE, scale);
  images->blocViolet = genererBloc(VIOLET, scale);
  images->blocOrange = genererBloc(ORANGE, scale);
  images->blocVert = genererBloc(VERT, scale);
  images->blocRouge = genererBloc(ROUGE, scale);
  images->blocBlanc = genererBloc(BLANC, scale);

  images->barreGrilleVerticale = al_create_bitmap((3* scale), (28 * scale));
  al_set_target_bitmap(images->barreGrilleVerticale);
  al_clear_to_color(al_map_rgb(0, 0, 0));

  images->barreGrilleHorizontale = al_create_bitmap((28 * scale), (3 * scale));
...
ALLEGRO_BITMAP *genererBloc(int typeDeBloc, float scale)
{
  ALLEGRO_BITMAP *bloc = al_create_bitmap(BLOC_SIZE *scale, BLOC_SIZE *scale);
  ...
  al_draw_filled_rectangle(3 * scale, 3 * scale, 23 * scale, 22 * scale, couleurPrincipale); //on dessine le rectangle central
  al_draw_filled_rectangle(3 * scale, 0 * scale, 25 * scale, 3 * scale, couleurClaire); //barre claire horizontale
  al_draw_filled_rectangle(22 * scale, 3 * scale, 25 * scale, 22 * scale, couleurClaire); //barre claire verticale

  al_draw_filled_rectangle(0 * scale, 0 * scale, 3 * scale, 22 * scale, couleurFoncee); //barre foncée horizontale
  al_draw_filled_rectangle(0 * scale, 22 * scale, 25 * scale, 25 * scale, couleurFoncee); //barre foncée verticale

  al_draw_filled_triangle(0 * scale, 0 * scale, 3 * scale, 0 * scale, 3 * scale, 3 * scale, couleurClaire); //triangle clair gauche
  al_draw_filled_triangle(22 * scale, 22 * scale, 25 * scale, 22 * scale, 25 * scale, 25 * scale, couleurClaire); //triangle clair droit
