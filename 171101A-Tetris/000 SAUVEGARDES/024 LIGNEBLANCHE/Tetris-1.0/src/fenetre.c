...
void blitBlock(int block, gameScreen *screen, packPartie *images, bool *buffed, float x, float y)
{
  ...
  if(!(*buffed)) //si le bloc n'a pas en core été dessiné dans le buffer
  {
  ...
  al_draw_bitmap_region(images->gridBuffer, x - (scale * 87), y - (scale * 22), 31 * scale, 31 * scale, x - (3 * scale), y - (3 * scale), 0);
}
