...
void testRes(int resolutions[4][2])
{
  ...
  resolutions[RESMIN][WIDTH] = resMinW;
  resolutions[RESMIN][HEIGHT] = resMinH;
  resolutions[RESMAX][WIDTH] = resMaxW;
  resolutions[RESMAX][HEIGHT] = resMaxH;

  if((SCREEN_W < resolutions[RESMAX][WIDTH] && SCREEN_H < resolutions[RESMAX][HEIGHT]) && (SCREEN_W > resolutions[RESMIN][WIDTH] && SCREEN_H > resolutions[RESMIN][HEIGHT]))
  {
    resolutions[RESDEF][WIDTH] = SCREEN_W;
    resolutions[RESDEF][HEIGHT] = SCREEN_H;
  }
  else if(resolutions[RESMIN][WIDTH] > 0 && resolutions[RESMIN][WIDTH] > 0)
  {
    resolutions[RESDEF][WIDTH] = resolutions[RESMIN][WIDTH];
    resolutions[RESDEF][WIDTH] = resolutions[RESMIN][WIDTH];
  }
  else
  {
  ...
  resolutions[RESCUR][WIDTH] = resolutions[RESDEF][WIDTH];
  resolutions[RESCUR][HEIGHT] = resolutions[RESDEF][HEIGHT];

  printf("\nResolution Min: %dx%d\nResolution Max: %dx%d\n", resolutions[RESMIN][WIDTH], resolutions[RESMIN][HEIGHT], resolutions[RESMAX][WIDTH], resolutions[RESMAX][HEIGHT]);
