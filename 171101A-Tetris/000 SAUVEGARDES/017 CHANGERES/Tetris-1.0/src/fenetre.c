...
gameScreen *genererGameScreen(gameConfig *config)
{
  ...
  screen->buffW = screen->resW * scale;
  screen->buffH = screen->resH * scale;
...
void changeRes(gameScreen *screen, elements *elem, packImages *images, gameConfig *config)
{
  ...
  screen->buffW = SCREEN_W * scale;
  screen->buffH = SCREEN_H * scale;
  ...
  resizeElements(elem, scale);

  //on redessine les images
  regenererPackImages(images, elem, ALL, 0, NULL, config->keyMap, 0, screen->buffW, screen->buffH);
}
