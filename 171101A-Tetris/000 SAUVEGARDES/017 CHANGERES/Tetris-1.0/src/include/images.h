...
packMenu *genererPackMenu(elements *elem, float buffW, float buffH); //on génère la structure d'images
void destroyPackMenu(packMenu *images); //on détruit la structure d'image
ALLEGRO_BITMAP *genererFondMenu(elements *elem, float buffW, float buffH); //va générer l'image de fond du menu principal
...
packPartie *genererPackPartie(elements *elem, int modeDeJeu, float buffW, float buffH); //on génère la structure d'images
void destroyPackPartie(packPartie *images); //on détruit la structure d'image
ALLEGRO_BITMAP *genererFondPartie(elements *elem, int modeDeJeu, float buffW, float buffH); //va générer l'image de fond de la partie
ALLEGRO_BITMAP *genererFondPause(elements *elem, ALLEGRO_BITMAP *fondPartie, float buffW, float buffH); //va générer l'image de fond du menu pause
...
packOptions *genererPackOptions(elements *elem, ALLEGRO_BITMAP *fondPrecedent, int keyMap[16][3], float buffW, float buffH); //on génère la structure d'images
...
ALLEGRO_BITMAP *genererFondInputOptions(elements *elem, ALLEGRO_BITMAP *fondPrecedent, float buffW, float buffH); //va générer l'image d'inputOptions
...
packSTLInput *genererPackSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu, float buffW, float buffH); //va générer le pack d'images de STLInput
void destroyPackSTLInput(packSTLInput *images); //va détruire la structure d'images de scoreInput
ALLEGRO_BITMAP *genererFondSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu, float buffW, float buffH); //va générer le fond de STLInput
...
//fonctions d'images
packImages *genererPackImages(elements *elem, int keyMap[16][3], float buffW, float buffH); //va générer les images du jeu à partir des coord d'elem
