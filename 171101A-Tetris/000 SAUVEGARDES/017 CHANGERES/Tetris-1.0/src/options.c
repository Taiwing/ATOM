...
void menuOptions(gameScreen *screen, gameConfig *config, elements *elem, packImages *images, packSons *sons)
{
  ...
  regenererPackImages(images, elem, PIOPTIONS, 0, screen->buffer, config->keyMap, 0, screen->buffW, screen->buffH);
      ...
      if(bouton != OFF && (ev.mouse.button & 1)) //si c'est le cas
      {
        switch(bouton)
        {
          ...
          default: //s'il est sur une zone d'input
            ...
            images->imagesOptions->fondInputOptions = genererFondInputOptions(elem, screen->buffer, screen->buffW, screen->buffH);
            al_set_target_bitmap(al_get_backbuffer(screen->display));
            al_uninstall_mouse();
