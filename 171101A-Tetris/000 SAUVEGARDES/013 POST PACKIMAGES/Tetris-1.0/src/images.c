...
packSTLInput *genererPackSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu)
{
  packSTLInput *images = malloc(sizeof(packSTLInput));

  images->fond = genererFondSTLInput(fondPrecedent, elem, score, modeDeJeu);
...
ALLEGRO_BITMAP *genererFondSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu)
{
...
packImages *genererPackImages(elements *elem, int keyMap[16][3])
{
  //allocation
  packImages *images = malloc(sizeof(packImages));

  //génération des images
  images->imagesMenu = genererPackMenu(elem);
  images->imagesPartie = genererPackPartie(elem, MARATHON);
  images->imagesOptions = genererPackOptions(elem, images->imagesMenu->fondMenu, keyMap);
  images->imagesSTLInput = genererPackSTLInput(NULL, elem, 0, 0);
}

void regenererPackImages(packImages *images, elements *elem, int regen, int modeDeJeu, ALLEGRO_BITMAP *fondPrecedent, int keyMap[16][3], int score)
{
  ...
  switch(regen)
  {
    ...
    case PIPARTIE:
      ...
      images->imagesPartie = genererPackPartie(elem, modeDeJeu);
      break;
    case PIOPTIONS:
      destroyPackOptions(images->imagesOptions);
      images->imagesOptions = genererPackOptions(elem, fondPrecedent, keyMap);
      break;
    case PISTLINPUT:
      destroyPackSTLInput(images->imagesSTLInput);
      images->imagesSTLInput = genererPackSTLInput(fondPrecedent, elem, score, modeDeJeu);
      break;
    case ALL:
