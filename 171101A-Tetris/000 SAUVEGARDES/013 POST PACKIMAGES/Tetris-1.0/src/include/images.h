...
packSTLInput *genererPackSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu); //va générer le pack d'images de STLInput
void destroyPackSTLInput(packSTLInput *images); //va détruire la structure d'images de scoreInput
ALLEGRO_BITMAP *genererFondSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu); //va générer le fond de STLInput
...
//fonctions d'images
packImages *genererPackImages(elements *elem, int keyMap[16][3]); //va générer les images du jeu à partir des coord d'elem
