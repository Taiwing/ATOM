...
void partieUltra(gameScreen *screen, int niveauChoisi, gameConfig *config, packSons *sons, elements *elem)
{
  ...
  if(data->victoire) //si le joueur a gagné (s'il a survécu jusqu'au bout du timer)
    highLignes(screen, data->lignes, elem); //on teste le nombre de lignes pour voir s'i lest dans le top 5
...
void afficherPartieUltra(gameScreen *screen, packPartie *images, elements *elem, gameData *data, int timerUltra)
{
