...
void menuPrincipal()
{
  ...
  //puis on charge les sons du jeu
  packSons *sons = chargerSon();
  ...
  bool sound[3] = {true, true, true}; //va indiquer si le son est activé ou non
  ...
  while(continuerMenu)
  {
      ...
      if(bouton != OFF) //si c'est le cas
      {
        switch(bouton)
        {
          case JOUER:
            ...
            {
              al_uninstall_mouse();
              partie(fenetre, niveau, sound, sons); //on lance la partie
              al_install_mouse();
    ...
    if(redraw && al_is_event_queue_empty(event_queue_menu)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      jouerSon(sound, sons, NULL); //va jouer ou arrêter le son du jeu
    }
  }
