...
void highScore(ALLEGRO_DISPLAY *fenetre, int score)
{
  ...
  if(place) //si le nouveau score est supérieur à l'un de ceux du high score (si place != 0)
  {
    ...
    scoreInput(fenetre, joueur, score); //le joueur rentre son nom (et le panneau de high score est affiché)
    nouveauScore(joueur, score, place); //on rentre les nouvelles données dans le high score
  }
}

void lireFichierScore(char J[6][4], char S[6][7])
{
  FILE *scores = fopen("highScore.sco", "r");

  char ligne[11] = ""; //variable qui va nous servir à lire le fichier

  for(int i = 0; i < 6; i++) //on répète la boucle six fois
  {
...
void nouveauScore(char joueur[4], int score, int place)
{
  ...
  char J[6][4], J2[6][4]; //va stocker le nom des joueurs
  char S[6][7], S2[6][7]; //va stocker les scores
  lireFichierScore(J, S); //on récupère les valeurs du high score
  lireFichierScore(J2, S2); //on récupère les valeurs du high score
  printf("lireFichierScore:\n");
  for(int i = 0; i < 6; i++)
  {
    printf("%s: %s\n", J2[i], S2[i]);
  }
  ...
  FILE *scores = fopen("highScore.sco", "w+"); //on lit et écrit dans le fichier en effaçant son contenu au préalable
...
{
  elementsFondScoreInput *elem = genererElementsFondScoreInput(); //on génére les éléments du fond de scoreInput
  packScoreInput *images = genererPackScoreInput(fenetre, elem, score); //on génère les images nécessaires
  ...
  al_register_event_source(event_queue_score, al_get_display_event_source(fenetre)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  ...
  while(continuer) //on lance la boucle du menu
  {
    ...
    else if(ev.type == ALLEGRO_EVENT_MOUSE_AXES) //si la souris bouge
    {
      ...
      boutonOk = estSurUnBoutonScore(elem->rectOk, ev.mouse.x, ev.mouse.y);
    }
    ...
    {
      redraw = false;
      afficherScoreInput(images, elem, boutonOk, joueur); //on affiche l'image de scoreInput à nouveau
  ...
  destroyElementsFondScoreInput(elem);
  destroyPackScoreInput(images);
}
