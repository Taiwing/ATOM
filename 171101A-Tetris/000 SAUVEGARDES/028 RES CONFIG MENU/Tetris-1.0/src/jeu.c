        ...
        case VERT_S:
          switch(ghostTetro[1][4])
          {
            case POS1:
              ...
              drawTransHorBarre(posGrille[X] * scale, (posGrille[Y] - 3) * scale, scale);
              break;
            case POS2:
              posTetro[X] += 28;
              posTetro[Y] += 28;
              ...
              drawTransVerBarre((posGrille[X] - 3) * scale, (posGrille[Y] + 31) * scale, scale);
              break;
            case POS3:
              posTetro[X] += 28;
              posGrille[X] += 28;
        ...
        case ROUGE_Z:
          switch(ghostTetro[1][4])
          {
            case POS1:
              ...
              drawTransHorBarre((posGrille[X] + 31) * scale, (posGrille[Y] - 3) * scale, scale);
              break;
            case POS2:
              posTetro[X] += (2 * 28);
              posTetro[Y] += 28;
              ...
              drawTransVerBarre((posGrille[X] + 28) * scale, (posGrille[Y] + 31) * scale, scale);
              break;
            case POS3:
              posTetro[Y] += (2 * 28);
              posGrille[Y] += posTetro[Y];
...
void afficherPartie(gameScreen *screen, packPartie *images, elements *elem, gameData *data, int modeDeJeu, int timer)
{
    ...
    case JAUNE_O:
      ...
      for(int y = 0; y < 4; y++)
      {
        ...
        for(int x = 0; x < 4; x++)
        {
          ...
          {
            al_draw_bitmap_region(images->blocJaune, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);
            switch(y)
            {
            ...
            if(dessinerBarreHaute)
              al_draw_bitmap_region(images->barreGrilleHorizontale, 0, 0, 28 * scale, 3 * scale, pos[X] - (3 * scale), pos[Y] - (3 * scale), 0);
            if(dessinerBarreGauche)
              al_draw_bitmap_region(images->barreGrilleVerticale, 0, 0, 3 * scale, 28 * scale, pos[X] - (3 * scale), pos[Y], 0);
            al_draw_bitmap_region(images->barreGrilleHorizontale, 0, 0, 28 * scale, 3 * scale, pos[X], pos[Y] + (25 * scale), 0);
            al_draw_bitmap_region(images->barreGrilleVerticale, 0, 0, 3 * scale, 28 * scale, pos[X] + (25 * scale), pos[Y] - (3 * scale), 0);
          }
          pos[X] += (28 * scale);
        }
        pos[Y] += (28 * scale);
    ...
    case CYAN_I:
      ...
      for(int y = 0; y < 4; y++)
      {
        ...
        for(int x = 0; x < 4; x++)
        {
          ...
          {
            al_draw_bitmap_region(images->blocCyan, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);
            switch(y)
            {
              case 0: //si on est sur une case en haut de la grille
                dessinerBarreHaute = true;
    ...
    case MAUVE_T:
      ...
      for(int y = 0; y < 4; y++)
      {
        ...
        for(int x = 0; x < 4; x++)
        {
          ...
          {
            al_draw_bitmap_region(images->blocMauve, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);;
            switch(y)
            {
              case 0: //si on est sur une case en haut de la grille
                dessinerBarreHaute = true;
    ...
    case VIOLET_J:
      ...
      for(int y = 0; y < 4; y++)
      {
        ...
        for(int x = 0; x < 4; x++)
        {
          ...
          {
            al_draw_bitmap_region(images->blocViolet, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);
            switch(y)
            {
              case 0: //si on est sur une case en haut de la grille
                dessinerBarreHaute = true;
    ...
    case ORANGE_L:
      ...
      for(int y = 0; y < 4; y++)
      {
        ...
        for(int x = 0; x < 4; x++)
        {
          ...
          {
            al_draw_bitmap_region(images->blocOrange, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);
            switch(y)
            {
              case 0: //si on est sur une case en haut de la grille
                dessinerBarreHaute = true;
    ...
    case VERT_S:
      ...
      for(int y = 0; y < 4; y++)
      {
        ...
        for(int x = 0; x < 4; x++)
        {
          ...
          {
            al_draw_bitmap_region(images->blocVert, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);
            switch(y)
            {
              case 0: //si on est sur une case en haut de la grille
                dessinerBarreHaute = true;
    ...
    case ROUGE_Z:
      ...
      for(int y = 0; y < 4; y++)
      {
        ...
        for(int x = 0; x < 4; x++)
        {
          ...
          {
            al_draw_bitmap_region(images->blocRouge, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);
            switch(y)
            {
              case 0: //si on est sur une case en haut de la grille
                dessinerBarreHaute = true;
  ...
  for(int y = 2; y < GRID_H; y++) //on va parcourir la grille à partir de la troisième ligne
  {
    ...
    for(int x = 0; x < GRID_W; x++) //on va parcourir la grille à partir de la deuxième colonne
    {
      ...
      {
        case JAUNE:
          if(data->animDisp && lignesDisp[y] && x == 0) //si cette ligne doit disparaître
            al_draw_bitmap_region(images->ligneBlanche, 0, 0, 283 * scale, 31 * scale, pos[X] - (3 * scale), pos[Y] - (3 * scale), 0);
          else if(!lignesDisp[y])
            al_draw_bitmap_region(images->blocJaune, 0, 0, 25 * scale, 25 * scale, pos[X], pos[Y], 0);
          switch(y)
          {
