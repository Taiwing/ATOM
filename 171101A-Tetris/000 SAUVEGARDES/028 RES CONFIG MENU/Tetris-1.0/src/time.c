...
void highTemps(gameScreen *screen, int temps, packImages *images, elements *elem)
{
  int place = testTemps(temps);
  if(place) //si le nouveau temps est supérieur à l'un de ceux du top (si place != 0)
  {
    char joueur[4]; //va stocker le nom du joueur
    tempsInput(screen, joueur, temps, images, elem); //le joueur rentre son nom (et le panneau de temps est affiché)
    nouveauTemps(joueur, temps, place); //on rentre les nouvelles données
  }
}

void lireFichierTemps(char J[5][4], char S[5][7])
{
  FILE *temps = fopen("game_files/temps.tim", "r");

  char ligne[10] = ""; //variable qui va nous servir à lire le fichier

  for(int i = 0; i < 5; i++) //on répète la boucle cinq fois
  {
    fgets(ligne, 10, temps); //on récupère la ligne
    for(int p = 0; p < 10; p++)
    {
      if(ligne[p] == '\n')
        ligne[p] = '\0';
    ...
    for(int k = 3; k < strlen(ligne) + 1; k++) //on récupère le temps
    {
      if(k < strlen(ligne))
        S[i][k-3] = ligne[k];
      else
...
int testTemps(int temps)
{
  ...
  char S[5][7]; //va stocker les temps
  lireFichierTemps(J, S); //on récupère les valeurs du top
...
{
  char strTemps[7];
  sprintf(strTemps, "%d", temps); //on récupère le nouveau temps dans un string (heu heu heu LOL)

  //on récupère les données du fichier en double
  char J[5][4], J2[5][4]; //va stocker le nom des joueurs
  char S[5][7], S2[5][7]; //va stocker les temps
  lireFichierTemps(J, S); //on récupère les valeurs
  lireFichierTemps(J2, S2); //on récupère les valeurs

  //on place le nouveau nom du joueur et le nouveau temps dans les tableaux récupérés dans le fichier
  for(int i = 0; i < 5; i++)
  {
    if(i == (place - 1)) //quand on arrive à l'endroit où stocker le nouveau temps
    {
      for(int j = 0; j < 4; j++)
      {
      ...
      for(int k = 0; k < 7; k++)
      {
        S2[i][k] = strTemps[k]; //puis le temps
      }
    }
    else if(i > (place - 1)) //quand on est après, on copie les anciens temps à leur nouvelle place
    {
      for(int j = 0; j < 4; j++)
      {
        J2[i][j] = J[i - 1][j];
  ...
  FILE *fTemps = fopen("game_files/temps.tim", "w+"); //on lit et écrit dans le fichier en effaçant son contenu au préalable

  //on y met le nouveau tableau de temps
  for(int i = 0; i < 5; i++)
  {
    fputs(J2[i], fTemps); //on met le nom du joueur
    fputs(S2[i], fTemps); //puis le temps
    fputc('\n', fTemps); //puis on saute une ligne
  }
...
void tempsInput(gameScreen *screen, char joueur[4], int temps, packImages *images, elements *elem)
{
  regenererPackImages(images, elem, PISTLINPUT, SPRINT, screen->buffer, NULL, temps, screen->scaleBuff); //on régénère le packSTLInput pour avoir la bonne image
  ...
  chargerListeElemFondTempsInput(screen, elem, lElem);
  ...
  ALLEGRO_TIMER *timer_temps = al_create_timer(1.0 / FPS); //on règle le timer à un soixantième de seconde

  //on crée la file d'évènements
  ALLEGRO_EVENT_QUEUE *event_queue_temps = al_create_event_queue();
  al_register_event_source(event_queue_temps, al_get_display_event_source(screen->display)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  al_register_event_source(event_queue_temps, al_get_timer_event_source(timer_temps)); //on lie la file au timer
  al_register_event_source(event_queue_temps, al_get_mouse_event_source()); //puis à la souris
  al_register_event_source(event_queue_temps, al_get_keyboard_event_source()); //puis au clavier

  //on démarre le timer
  al_start_timer(timer_temps);

  while(continuer) //on lance la boucle du menu
  {
    ALLEGRO_EVENT ev; //on crée la variable qui va enregistrer les évènements
    al_wait_for_event(event_queue_temps, &ev); //on attend qu'un évènement se produise
        ...
        if(key[i]) //si une touche a été appuyée
        {
          switch(i) //on détermine laquelle
          {
            ...
            case ALLEGRO_KEY_ENTER:
              continuer = false; //on enregistre le temps
              break;
    ...
    else if(ev.type == ALLEGRO_EVENT_MOUSE_AXES) //si la souris bouge
    {
      ...
      boutonOk = estSurUnBoutonTemps(lElem, ev.mouse.x, ev.mouse.y);
    }
    ...
    if(redraw && al_is_event_queue_empty(event_queue_temps)) //si redraw est vrai et que la file d'évènements est vide
    {
      redraw = false;
      afficherTempsInput(screen, images->imagesSTLInput, elem, boutonOk, joueur); //on affiche l'image de tempsInput à nouveau
    }
  }

  //on libère la mémoire
  detruireListe(lElem);
  al_destroy_timer(timer_temps);
  al_destroy_event_queue(event_queue_temps);
}
...
void afficherTempsInput(gameScreen *screen, packSTLInput *images, elements *elem, int boutonOk, char joueur[4])
{
  targetBuffer(screen);
  float scale = screen->scaleBuff;

  al_draw_bitmap(images->fond, 0, 0, 0); //on affiche le fond de tempsInput
  ...
  ALLEGRO_FONT *font = al_load_ttf_font("media_files/Gameplay.ttf", 20 * scale, 0); //on charge la police "Gameplay"
  al_draw_text(font, al_map_rgb(0, 0, 0), rect->x + ((rect->x2 - rect->x)/2), rect->y + (10 * scale), ALLEGRO_ALIGN_CENTRE, joueurAffiche);
...
void chargerListeElemFondTempsInput(gameScreen *screen, elements *elem, Liste *lElem)
{
