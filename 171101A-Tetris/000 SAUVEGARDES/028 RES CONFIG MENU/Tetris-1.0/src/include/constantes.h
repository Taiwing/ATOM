...
enum boutons {ON, OFF, JOUER, MARATHON, SPRINT, ULTRA, AIDE, OPTIONS, NIVEAU, REPRENDRE, QUITTER}; //liste des differents boutons du menu principal et du menu pause
//liste des différents boutons du menu options
enum butonsOptions {OK = 8, IROTATER, ISOFTD, ILEFT, IRIGHT, IHARDD, IROTATEL, IPAUSE, IGHOST, ICHANGE, IRESW, IRESH, VOLTHEMEM, VOLTHEMEP, VOLFXM, VOLFXP, BRESMIN, BRESMAX, APPLIQUER, RESET};
...
enum touchesGlob {FULLSCREENSWITCH, FULLSCREENOFF}; //liste des commandes générales du programme
enum coordonnes {X, Y}; //permet d'accéder à la position de des tetrominos
enum gameEv {NOEVENT, GAMESTART, GAMEOVER, LEVELUP, FALL, HARDDROP, HOLD, LOCKED, MOVELR, ROTATEFAIL, ROTATELR, SOFTDROP, TOUCHDOWN, TOUCHLR, LCSINGLE, LCDOUBLE, LCTRIPLE, LCTETRIS, STSPIN, STSPINSINGLE, STSPINDOUBLE, STSPINTRIPLE}; //liste des évènements déclenchants un son
enum afficheEv {ALEVELUP, ALCSINGLE, ALCDOUBLE, ALCTRIPLE, ATETRIS, ATSPIN, ATSPINSINGLE, ATSPINDOUBLE, ATSPINTRIPLE};
enum soundOptions {SOUND, THEME, FX}; //liste des options de son
enum packImagesGLOBAL {PIMENU = 1, PIPARTIE, PIOPTIONS, PISTLINPUT, ALL}; //listes de flags de la fonction "regenererPackImages"
//va permettre de parcourir le tableau de résolutions
enum resolutions {RESMIN, RESMAX, RESDEF, RESCUR};
enum resolutionData {WIDTH, HEIGHT};
