...
void partieMarathon(ALLEGRO_DISPLAY *fenetre, int niveauChoisi, gameConfig *config, packSons *sons)
{
  ...
  if(config->sound->soundOnOff[0])
  {
    ...
    jouerSon(config->sound, sons, NULL, false); //va arrêter le son du jeu
  ...
  al_register_event_source(event_queue_partieMarathon, al_get_display_event_source(fenetre)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  ...
  al_register_event_source(event_queue_partieMarathon, al_get_keyboard_event_source()); //et au clavier
  ...
  while(data->continuer)
  {
    ...
    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        gameInput(data, fenetre, elem, images, timer_partieMarathon, sons, config); //gestion des input
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieMarathon)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      jouerSon(config->sound, sons, data->soundEv, false); //va jouer ou arrêter le son du jeu
    }
  }
  ...
  al_unregister_event_source(event_queue_partieMarathon, al_get_display_event_source(fenetre)); //devrait éviter l'erreur de SEGMENTATION
    ...
    if(data->key[i]) //si une touche a été appuyée
    {
      switch(i) //on détermine laquelle
      {
        ...
        case PAUSE:
          ...
          al_stop_timer(timer_partieMarathon); //on stoppe le timer pour que le temps passé en pause ne soit pas compté
          if(menuPause(fenetre, elem, images->imagesPause, data->niveau, data->score, data->lignes, sons, config)) //si le joueur a fait quitter
          ...
          else
          {
            ...
            al_start_timer(timer_partieMarathon); //on relance le timer
          }
          break;
        case GHOST:
          if(!data->ghostPieceOnOff)
