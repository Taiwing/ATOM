...
void partieUltra(gameScreen *screen, packImages *images, int niveauChoisi, gameConfig *config, packSons *sons, elements *elem)
{
  //on régénère le fond de la partie
  regenererPackImages(images, elem, PIPARTIE, ULTRA, NULL, config->keyMap, 0, screen->buffW, screen->buffH);
  ...
  while(data->continuer)
  {
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieUltra)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieUltra(screen, images->imagesPartie, elem, data, timerUltra); //on affiche la partie à nouveau
  ...
  if(data->victoire) //si le joueur a gagné (s'il a survécu jusqu'au bout du timer)
    highLignes(screen, data->lignes, images, elem); //on teste le nombre de lignes pour voir s'i lest dans le top 5
