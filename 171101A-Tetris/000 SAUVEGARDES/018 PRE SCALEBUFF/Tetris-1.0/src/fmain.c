...
void menuPrincipal()
{
  ...
  packImages *images = genererPackImages(elem, config->keyMap, screen->buffW, screen->buffH); //on génère les images du jeu
            ...
            if(ev.mouse.button & 1) //s'il y a un clic gauche sur ce bouton
            {
              switch(modeDeJeu)
              {
                case MARATHON:
                  ...
                  images->imagesMenu->highScore = genererHigh(MARATHON, elem);
                ...
                case SPRINT:
                  ...
                  images->imagesMenu->highTemps = genererHigh(SPRINT, elem);
                  //on régénère également le pack options pour afficher les changements s'il y en a eu
                  break;
                case ULTRA:
                  al_uninstall_mouse();
                  ...
                  images->imagesMenu->highLines = genererHigh(ULTRA, elem);
                  //on régénère également le pack options pour afficher les changements s'il y en a eu
                  break;
              }
            }
