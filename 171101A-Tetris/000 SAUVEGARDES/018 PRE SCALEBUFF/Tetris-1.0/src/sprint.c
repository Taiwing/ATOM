...
void partieSprint(gameScreen *screen, packImages *images, int niveauChoisi, gameConfig *config, packSons *sons, elements *elem)
{
  //on régénère le fond de la partie
  regenererPackImages(images, elem, PIPARTIE, SPRINT, NULL, config->keyMap, 0, screen->buffW, screen->buffH);
  ...
  while(data->continuer)
  {
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieSprint)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieSprint(screen, images->imagesPartie, elem, data, timerSprint); //on affiche la partie à nouveau
      jouerSon(config->sound, sons, data->gameEvent, false); //va jouer ou arrêter le son du jeu
    }
  }

  if(data->victoire) //si le joueur a gagné
    highTemps(screen, timerSprint, images, elem); //on va sauvegarder les meilleurs temps comme pour les scores de marathon
