...
packMenu *genererPackMenu(elements *elem, float buffW, float buffH)
{
  ...
  images->fondMenuAide = genererFondMenuAide(elem, true, buffW, buffH);
...
ALLEGRO_BITMAP *genererFondMenu(elements *elem, float buffW, float buffH)
{
  //on crée une image aux dimensions de l'écran dans laquelle on va dessiner le menu Principal
  ALLEGRO_BITMAP *fondMenu = al_create_bitmap(buffW, buffW);
...
packPartie *genererPackPartie(elements *elem, int modeDeJeu, float buffW, float buffH)
{
  //on crée un pointeur vers la structure d'images
  packPartie *images = malloc(sizeof(packPartie));

  //puis on initie chacune des images
  images->fondPartie = genererFondPartie(elem, modeDeJeu, buffW, buffH);
  ...
  images->imagesPause->fondPause = genererFondPause(elem, images->fondPartie, buffW, buffH);
  ...
  images->imagesPause->fondMenuAide = genererFondMenuAide(elem, false, buffW, buffH);
...
ALLEGRO_BITMAP *genererFondPartie(elements *elem, int modeDeJeu, float buffW, float buffH)
{
  //on crée un image aux dimensions de l'écran dans laquelle on va dessiner la partie
  ALLEGRO_BITMAP *fondPartie = al_create_bitmap(buffW, buffW);
...
ALLEGRO_BITMAP *genererFondPause(elements *elem, ALLEGRO_BITMAP *fondPartie, float buffW, float buffH)
{
  //on crée un image aux dimensions de l'écran dans laquelle on va dessiner la pause
  ALLEGRO_BITMAP *fondPause = al_create_bitmap(buffW, buffW);
...
ALLEGRO_BITMAP *genererFondMenuAide(elements *elem, bool menu, float buffW, float buffH)
{
  //on crée un image aux dimensions de l'écran dans laquelle on va dessiner le menu d'Aide
  ALLEGRO_BITMAP *fondAide = al_create_bitmap(buffW, buffW);
...
packOptions *genererPackOptions(elements *elem, ALLEGRO_BITMAP *fondPrecedent, int keyMap[16][3], float buffW, float buffH)
{
  //on crée un pointeur vers la structure d'images
  packOptions *images = malloc(sizeof(packOptions));

  //puis on initie chacune des images
  images->fondOptions = genererFondOptions(elem, fondPrecedent, 0, buffW, buffH);
  ...
  images->fondInputOptions = genererFondInputOptions(elem, images->fondOptions, buffW, buffH);
...
ALLEGRO_BITMAP *genererFondOptions(elements *elem, ALLEGRO_BITMAP *fondPrecedent, int recharge, float buffW, float buffH)
{
  //on crée un image aux dimensions de l'écran dans lequel on va dessiner le menu d'options
  ALLEGRO_BITMAP *fondOptions = al_create_bitmap(buffW, buffW);
...
ALLEGRO_BITMAP *genererFondInputOptions(elements *elem, ALLEGRO_BITMAP *fondPrecedent, float buffW, float buffH)
{
  //on crée un image aux dimensions de l'écran dans laquelle on va dessiner l'inputOptions
  ALLEGRO_BITMAP *fondInputOptions = al_create_bitmap(buffW, buffW);
...
packSTLInput *genererPackSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu, float buffW, float buffH)
{
  packSTLInput *images = malloc(sizeof(packSTLInput));

  images->fond = genererFondSTLInput(fondPrecedent, elem, score, modeDeJeu, buffW, buffH);
...
ALLEGRO_BITMAP *genererFondSTLInput(ALLEGRO_BITMAP *fondPrecedent, elements *elem, int score, int modeDeJeu, float buffW, float buffH)
{
  ALLEGRO_BITMAP *fond = al_create_bitmap(buffW, buffW); //on crée l'image de fond
  ...
  switch(modeDeJeu)
  {
    ...
    default: //si c'est la première fois qu'on génère le pack d'images
      al_draw_text(font, bleuActif, rect->x + ((rect->x2 - rect->x)/2), rect->y + 30, ALLEGRO_ALIGN_CENTRE, "NADA");
      break;
  }
...
packImages *genererPackImages(elements *elem, int keyMap[16][3], float buffW, float buffH)
{
  //allocation
  packImages *images = malloc(sizeof(packImages));

  //génération des images
  images->imagesMenu = genererPackMenu(elem, buffW, buffH);
  images->imagesPartie = genererPackPartie(elem, MARATHON, buffW, buffH);
  images->imagesOptions = genererPackOptions(elem, images->imagesMenu->fondMenu, keyMap, buffW, buffH);
  images->imagesSTLInput = genererPackSTLInput(NULL, elem, 0, 0, buffW, buffH);
}

void regenererPackImages(packImages *images, elements *elem, int regen, int modeDeJeu, ALLEGRO_BITMAP *fondPrecedent, int keyMap[16][3], int score, float buffW, float buffH)
{
  ...
  switch(regen)
  {
    ...
    case PIPARTIE:
      ...
      images->imagesPartie = genererPackPartie(elem, modeDeJeu, buffW, buffH);
      break;
    case PIOPTIONS:
      destroyPackOptions(images->imagesOptions);
      images->imagesOptions = genererPackOptions(elem, fondPrecedent, keyMap, buffW, buffH);
      break;
    case PISTLINPUT:
      destroyPackSTLInput(images->imagesSTLInput);
      images->imagesSTLInput = genererPackSTLInput(fondPrecedent, elem, score, modeDeJeu, buffW, buffH);
      break;
    case ALL:
