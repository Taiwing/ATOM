...
void partieMarathon(ALLEGRO_DISPLAY *fenetre, int niveauChoisi, gameConfig *config, packSons *sons)
{
  ...
  packPartie *images = genererPackPartie(elem, MARATHON); //on crée la structure d'où on va tirer les images de la partie
  ...
  while(data->continuer)
  {
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieMarathon)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieMarathon(images, elem, data); //on affiche la partie à nouveau
...
void afficherPartieMarathon(packPartie *images, elementsFondPartie *elem, gameData *data)
{
