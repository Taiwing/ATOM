...
void partieSprint(ALLEGRO_DISPLAY *fenetre, int niveauChoisi, gameConfig *config, packSons *sons)
{
  ...
  al_register_event_source(event_queue_partieSprint, al_get_display_event_source(fenetre)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  ...
  while(data->continuer)
  {
    ...
    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        gameInput(data, fenetre, elem, images, timer_partieSprint, sons, config); //gestion des input
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieSprint)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieSprint(images, elem, data, timerSprint); //on affiche la partie à nouveau
  ...
  if(data->victoire) //si le joueur a gagné
    highTemps(fenetre, timerSprint); //on va sauvegarder les meilleurs temps comme pour les scores de marathon
  ...
  al_unregister_event_source(event_queue_partieSprint, al_get_display_event_source(fenetre)); //devrait éviter l'erreur de SEGMENTATION
  al_destroy_event_queue(event_queue_partieSprint);
}

void afficherPartieSprint(packPartie *images, elementsFondPartie *elem, gameData *data, int timerSprint)
{
