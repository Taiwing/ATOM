...
void highLignes(ALLEGRO_DISPLAY *fenetre, int lignes)
{
  ...
  if(place) //si le nouveau lignes est supérieur à l'un de ceux du top (si place != 0)
  {
    ...
    lignesInput(fenetre, joueur, lignes); //le joueur rentre son nom (et le panneau de lignes est affiché)
...
void lignesInput(ALLEGRO_DISPLAY *fenetre, char joueur[4], int lignes)
{
  ...
  packLignesInput *images = genererPackLignesInput(fenetre, elem, lignes); //on génère les images nécessaires
  ...
  al_register_event_source(event_queue_lignes, al_get_display_event_source(fenetre)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  ...
  while(continuer) //on lance la boucle du menu
  {
    ...
    else if(ev.type == ALLEGRO_EVENT_MOUSE_AXES) //si la souris bouge
    {
      ...
      boutonOk = estSurUnBoutonLignes(elem->rectOk, ev.mouse.x, ev.mouse.y);
    }
    ...
    {
      redraw = false;
      afficherLignesInput(images, elem, boutonOk, joueur); //on affiche l'image de lignesInput à nouveau
