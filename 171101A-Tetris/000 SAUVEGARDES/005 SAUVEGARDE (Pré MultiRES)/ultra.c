...
void partieUltra(ALLEGRO_DISPLAY *fenetre, int niveauChoisi, gameConfig *config, packSons *sons)
{
  ...
  al_register_event_source(event_queue_partieUltra, al_get_display_event_source(fenetre)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  ...
  while(data->continuer)
  {
    ...
    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        gameInput(data, fenetre, elem, images, timer_partieUltra, sons, config); //gestion des input
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieUltra)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieUltra(images, elem, data, timerUltra); //on affiche la partie à nouveau
  ...
  if(data->victoire) //si le joueur a gagné (s'il a survécu jusqu'au bout du timer)
    highLignes(fenetre, data->lignes); //on teste le nombre de lignes pour voir s'i lest dans le top 5
  ...
  al_unregister_event_source(event_queue_partieUltra, al_get_display_event_source(fenetre)); //devrait éviter l'erreur de SEGMENTATION
  al_destroy_event_queue(event_queue_partieUltra);
}

void afficherPartieUltra(packPartie *images, elementsFondPartie *elem, gameData *data, int timerUltra)
{
