...
void highTemps(ALLEGRO_DISPLAY *fenetre, int temps)
{
  ...
  if(place) //si le nouveau temps est supérieur à l'un de ceux du top (si place != 0)
  {
    ...
    tempsInput(fenetre, joueur, temps); //le joueur rentre son nom (et le panneau de temps est affiché)
...
void nouveauTemps(char joueur[4], int temps, int place)
{
  ...
  FILE *fTemps = fopen("temps.tim", "w+"); //on lit et écrit dans le fichier en effaçant son contenu au préalable
...
void tempsInput(ALLEGRO_DISPLAY *fenetre, char joueur[4], int temps)
{
  ...
  packTempsInput *images = genererPackTempsInput(fenetre, elem, temps); //on génère les images nécessaires
  ...
  al_register_event_source(event_queue_temps, al_get_display_event_source(fenetre)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  ...
  while(continuer) //on lance la boucle du menu
  {
    ...
    else if(ev.type == ALLEGRO_EVENT_MOUSE_AXES) //si la souris bouge
    {
      ...
      boutonOk = estSurUnBoutonTemps(elem->rectOk, ev.mouse.x, ev.mouse.y);
    }
    ...
    {
      redraw = false;
      afficherTempsInput(images, elem, boutonOk, joueur); //on affiche l'image de tempsInput à nouveau
...
{
  if(x >= OK->x && x <= OK->x2 && y >= OK->y && y <= OK->y2)//si elle est sur le bouton de ok
    return ON;
