...
void menuPrincipal()
{
  ...
  while(continuerMenu)
  {
    ...
    else if(ev.type == ALLEGRO_EVENT_MOUSE_AXES) //si la souris bouge
    {
      ...
      bouton = estSurUnBouton(elem, ev.mouse.x, ev.mouse.y, modeDeJeu);
    }
          ...
          case JOUER:
            if(ev.mouse.button & 1) //s'il y a un clic gauche sur ce bouton
            {
              switch(modeDeJeu)
              {
                case MARATHON:
                  al_uninstall_mouse();
                  partieMarathon(fenetre, niveau, config, sons); //on lance la partie
                  al_install_mouse();
                ...
                case SPRINT:
                  al_uninstall_mouse();
                  partieSprint(fenetre, niveau, config, sons); //on lance la partie
                  al_install_mouse();
                  al_register_event_source(event_queue_menu, al_get_mouse_event_source()); //puis on reprend les évènements de la souris en compte
                ...
                case ULTRA:
                  al_uninstall_mouse();
                  partieUltra(fenetre, niveau, config, sons); //on lance la partie
                  al_install_mouse();
                  al_register_event_source(event_queue_menu, al_get_mouse_event_source()); //puis on reprend les évènements de la souris en compte
                  //puis on recharge l'image du highScore
    ...
    {
      redraw = false;
      afficherMenuPrincipal(images, elem, bouton, niveau, modeDeJeu); //on affiche le menu à nouveau
...
int estSurUnBouton(elementsFondMenu *elem, int x, int y, int modeDeJeu)
{
  if(x >= elem->rectJouer->x && x <= elem->rectJouer->x2 && y >= elem->rectJouer->y && y <= elem->rectJouer->y2)//si la souris est sur le bouton Jouer
    return JOUER;
  if(x >= elem->rectMode->x && x <= elem->rectMode->x2 && y >= elem->rectMode->y && y <= elem->rectMode->y2)//si elle est sur le bouton de mode
    return modeDeJeu;
...
void afficherMenuPrincipal(packMenu *images, elementsFondMenu *elem, int boutonOn, int niveau, int modeDeJeu)
{
