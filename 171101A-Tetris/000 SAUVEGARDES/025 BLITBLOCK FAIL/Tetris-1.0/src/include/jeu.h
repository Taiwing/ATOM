...
struct gameData
{
  ...
  //va indiquer les coordonnées des lignes disparues (et 0 pour une ligne toujours là) pour la fonction d'affichage
  int dispLines[4];
  bool actualizeGridBuffer; //va indiquer à la fonction d'affichage qu'il faut mettre à jour le buffer de la grille
};
...
void ghostPiece(packPartie *images, gameData *data, float scale); //va afficher le tétro fantôme
...
void gameInput(gameData *data, gameScreen *screen, elements *elem, packImages *images, ALLEGRO_EVENT_QUEUE *event_queue_partie, ALLEGRO_TIMER *timer_partieMarathon, packSons *sons, gameConfig *config);
