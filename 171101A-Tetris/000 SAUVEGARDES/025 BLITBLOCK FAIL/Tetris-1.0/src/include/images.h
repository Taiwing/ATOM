...
struct packPartie
{
  ...
  ALLEGRO_BITMAP *ligneBlanche;
  ALLEGRO_BITMAP *upLines;
  ALLEGRO_BITMAP *gridBuffer; //image qu'on va dessiner sur le buffer et qui va contenir la grille de jeu (taille: ceil(rectPrincipal*scale))
  bool buffingGrid[GRID_W][GRID_H]; //grille qui va dire à la fonction d'affichage si les blocks sont dessinés ou non dans "gridBuffer"
...
ALLEGRO_BITMAP *genererGridBuffer(elements *elem); //va générer l'image de la grille de jeu
ALLEGRO_BITMAP *genererLigneBlanche(float scale); //va générer l'image de "ligneDisp"
