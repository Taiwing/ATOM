...
void targetBuffer(gameScreen *screen)
{
  ...
  al_draw_bitmap_region(screen->fondDuJeu, screen->scaleX, screen->scaleY, screen->buffH, screen->buffW, 0, 0, 0); //on y dessine le fondDuJeu
}
...
void blitBlock(int block, gameScreen *screen, packPartie *images, bool *buffed, float x, float y)
{
  float scale = screen->scaleBuff;
  if(!(*buffed)) //si le bloc n'a pas encore été dessiné dans le buffer
  {
    al_set_target_bitmap(images->gridBuffer); //on cible le buffer de la grille
    drawBlock(block, x - (scale * 87), y - (scale * 22), scale); //on dessine le block dessus
    *buffed = true; //on met la case du tableau à vrai pour indiquer à la fonction que le block est dessiné
    al_set_target_bitmap(screen->buffer); //on cible le buffer général
  }
  //puis on affiche le bloc
  al_draw_bitmap_region(images->gridBuffer, x - (scale * 87), y - (scale * 22), 25 * scale, 25 * scale, x, y, 0);
}

void actualizeBuffingGrid(bool buffingGrid[GRID_W][GRID_H], int dispLines[4], gameScreen *screen, packPartie *images)
{
  float scale = screen->scaleBuff;
  int val = 0; //va parcourir le tableau dispLines

  for(int i = 0; i < GRID_H; i++) //on va parcourir le tableau de haut en bas
  {
    if(dispLines[val] == i) //si la ligne en question a disparu
    {
      //on blitte le dessus de l'image sur la ligne disparue
      al_set_target_bitmap(images->upLines); //on cible le nouveau buffer pour les dessins
      al_clear_to_color(al_map_rgb(0, 0, 0));
      al_draw_bitmap_region(images->gridBuffer, 0, 0, 280 * scale, ((i+1) * 28)  * scale, 0, 0, 0);
      al_set_target_bitmap(images->gridBuffer); //on recible le buffer de la grille
      al_draw_bitmap_region(images->upLines, 0, 0, 280 * scale, ((i+1) * 28) * scale, 0, 28 * scale, 0);

      //puis on met à jour le tableau du buffer
      for(int j = i; j > 0; j--) //on répète la boucle autant de fois qu'il y a de lignes au dessus de la complète
      {
        for(int k = 0; k < 10; k++) //on répète celle-ci une fois pour chaque bloc de la ligne (donc 10 fois)
        {
          buffingGrid[k][j] = buffingGrid[k][j-1]; //on affecte la valeur de la case du dessus à celle de la ligne complète
        }
      }
      val++; //on passe à la prochaine ligne disparue
    }
  }
}
...
void changeRes(gameScreen *screen, elements *elem, packImages *images, gameConfig *config)
{
  //d'abord on détermine le facteur de scalage pour redimensionner
  ...
  if((screen->resW == config->res[RESMAX][WIDTH] && screen->resH == config->res[RESMAX][HEIGHT]) && !(al_get_display_flags(screen->display) & ALLEGRO_FULLSCREEN_WINDOW))
    ...
    al_resize_display(screen->display, screen->resW, screen->resH); //sinon on redimensionne la fenêtre par la résolution voulue
