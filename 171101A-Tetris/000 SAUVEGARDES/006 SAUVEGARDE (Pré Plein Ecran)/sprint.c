...
void partieSprint(gameScreen *screen, int niveauChoisi, gameConfig *config, packSons *sons)
{
  ...
  al_register_event_source(event_queue_partieSprint, al_get_keyboard_event_source()); //et au clavier
  ...
  while(data->continuer)
  {
    ...
    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        gameInput(data, screen, elem, images, timer_partieSprint, sons, config); //gestion des input
