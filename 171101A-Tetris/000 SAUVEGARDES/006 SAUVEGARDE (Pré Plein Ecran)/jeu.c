...
void troisDeuxUn(gameScreen *screen, packPartie *images, elementsFondPartie *elem, int pause, int niveau, int score, int lignes)
{
...
void afficherTroisDeuxUn(gameScreen *screen, packPartie *images, elementsFondPartie *elem, int pause, int niveau, int score, int lignes, int temps)
{
...
void gameInput(gameData *data, gameScreen *screen, elementsFondPartie *elem, packPartie *images, ALLEGRO_TIMER *timer_partie, packSons *sons, gameConfig *config)
{
    ...
    if(data->key[i]) //si une touche a été appuyée
    {
      switch(i) //on détermine laquelle
      {
        ...
        case PAUSE:
          ...
          if(menuPause(screen, elem, images->imagesPause, data->niveau, data->score, data->lignes, sons, config)) //si le joueur a fait quitter
            data->continuer = false; //on revient au menu principal
          else
          {
            troisDeuxUn(screen, images, elem, 1, data->niveau, data->score, data->lignes); //on lance le compte à rebours avant de reprendre
...
void afficherEvenement(gameData *data, elementsFondPartie *elem)
{
  ...
  for(int i = 0; i < 9; i++)
  {
    if(data->afficherGameEvent[i])
    {
      ...
      ALLEGRO_FONT *font = al_load_ttf_font("Raleway-Bold.ttf", 30, 0);
