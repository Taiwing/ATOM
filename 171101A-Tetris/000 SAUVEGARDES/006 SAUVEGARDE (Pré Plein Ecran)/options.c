...
void menuOptions(gameScreen *screen, gameConfig *config, elementsFondOptions *elem, packOptions *images, packSons *sons)
{
  ...
  while(continuer) //on lance la boucle du menu
  {
      ...
      if(bouton != OFF && (ev.mouse.button & 1)) //si c'est le cas
      {
        switch(bouton)
        {
          ...
          case RESET:
            ...
            images = genererPackOptions(elem, al_get_backbuffer(screen->display), config->keyMap);
          ...
          default: //s'il est sur une zone d'input
            ...
            images->fondInputOptions = genererFondInputOptions(elem, screen->buffer);
            al_set_target_bitmap(al_get_backbuffer(screen->display));
            al_uninstall_mouse();
            inputOptions(screen, bouton, config, elem, images);
            al_install_mouse();
            ...
            //on régénère le pack options pour afficher les changements
            images = genererPackOptions(elem, screen->buffer, config->keyMap);
            al_set_target_bitmap(al_get_backbuffer(screen->display));
            break;
        }
      }
    ...
    {
      redraw = false;
      afficherOptions(screen, images, elem, bouton, vol); //on affiche le menu d'options à nouveau
...
void afficherOptions(gameScreen *screen, packOptions *images, elementsFondOptions *elem, int bouton, int vol[2])
{
...
void inputOptions(gameScreen *screen, int bouton, gameConfig *config, elementsFondOptions *elem, packOptions *images)
{
  //on charge la liste d'éléments pour la fonction "estSurUnBouton"
  Liste *lElem = initialisationListe();
...
void afficherInputOptions(gameScreen *screen, packOptions *images, elementsFondOptions *elem, int bouton, int touche, int zoneInput, gameConfig *config, int input[3])
{
  //on cible le buffer du jeu sur lequel on va dessiner
  al_set_target_bitmap(screen->buffer);
  al_clear_to_color(al_map_rgb(0, 0, 0));
...
void chargerListeElemFondOptions(gameScreen *screen, elementsFondOptions *elem, Liste *lElem)
{
  coord *rect = NULL;
  for(int i = 0; i < 15; i++)
  {
...
void chargerListeElemInputOptions(gameScreen *screen, elementsFondOptions *elem, Liste *lElem)
{
  coord *rect = NULL;
  for(int i = 0; i < 4; i++)
  {
