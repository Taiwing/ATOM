...
void highScore(gameScreen *screen, int score)
{
  ...
  if(place) //si le nouveau score est supérieur à l'un de ceux du high score (si place != 0)
  {
    ...
    scoreInput(screen, joueur, score); //le joueur rentre son nom (et le panneau de high score est affiché)
...
void scoreInput(gameScreen *screen, char joueur[4], int score)
{
  elementsFondScoreInput *elem = genererElementsFondScoreInput(); //on génére les éléments du fond de scoreInput
  packScoreInput *images = genererPackScoreInput(screen->buffer, elem, score); //on génère les images nécessaires
  ...
  while(continuer) //on lance la boucle du menu
  {
    ...
    {
      redraw = false;
      afficherScoreInput(screen, images, elem, boutonOk, joueur); //on affiche l'image de scoreInput à nouveau
    }
  }
...
void afficherScoreInput(gameScreen *screen, packScoreInput *images, elementsFondScoreInput *elem, int boutonOk, char joueur[4])
{
...
void chargerListeElemFondScoreInput(gameScreen *screen, elementsFondScoreInput *elem, Liste *lElem)
{
