...
void partieMarathon(gameScreen *screen, int niveauChoisi, gameConfig *config, packSons *sons)
{
  ...
  while(data->continuer)
  {
    ...
    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        gameInput(data, screen, elem, images, timer_partieMarathon, sons, config); //gestion des input
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieMarathon)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieMarathon(screen, images, elem, data); //on affiche la partie à nouveau
      jouerSon(config->sound, sons, data->gameEvent, false); //va jouer ou arrêter le son du jeu
    }
  }

  highScore(screen, data->score);
...
void afficherPartieMarathon(gameScreen *screen, packPartie *images, elementsFondPartie *elem, gameData *data)
{
