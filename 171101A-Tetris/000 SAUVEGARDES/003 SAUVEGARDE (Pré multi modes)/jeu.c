  ...
  while(data->continuer)
  {
    ...
    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        gameInput(data, fenetre, elem, images, timer_partie, sons, config); //gestion des input
...
void gameInput(gameData *data, ALLEGRO_DISPLAY *fenetre, elementsFondPartie *elem, packPartie *images, ALLEGRO_TIMER *timer_partie, packSons *sons, gameConfig *config)
{
