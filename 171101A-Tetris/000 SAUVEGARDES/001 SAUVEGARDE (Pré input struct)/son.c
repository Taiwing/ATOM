...
void jouerSon(bool sound[3], packSons *sons, File *soundEv)
{
  if(sound[SOUND] && sound[THEME] && (!al_get_sample_instance_playing(sons->theme))) //si le son est activé et que le thème ne joue plus
  {
    al_play_sample_instance(sons->theme); //on joue le thème
  }
  else if((!sound[SOUND] || !sound[THEME]) && (al_get_sample_instance_playing(sons->theme))) //si le son est desactivé et que le thème joue
  {
    al_stop_sample_instance(sons->theme); //on arrête de jouer le thème
  }

  if(sound[SOUND] && sound[FX] && soundEv != NULL) //si le son est activé et qu'il y a un son à jouer
  {
