        ...
        switch(bouton)
        {
          ...
          case AIDE:
            if(ev.mouse.button & 1) //s'il y a un clic gauche sur ce bouton
            {
              ...
              al_uninstall_mouse();
              menuAide(screen, images->fondMenuAide, images->boutonOkOn, images->boutonOkOff, elem->rectAide, elem->rectBAideOk, elem->rectNSL, niveau, score, lignes);
              al_install_mouse();
          ...
          case OPTIONS:
            if(ev.mouse.button & 1) //s'il y a un clic gauche sur ce bouton
            {
              ...
              packOptions *images2 = genererPackOptions(elem, screen->buffer, config->keyMap);
