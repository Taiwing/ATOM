...
void highTemps(gameScreen *screen, int temps, elements *elem)
{
  ...
  if(place) //si le nouveau temps est supérieur à l'un de ceux du top (si place != 0)
  {
    ...
    tempsInput(screen, joueur, temps, elem); //le joueur rentre son nom (et le panneau de temps est affiché)
...
void tempsInput(gameScreen *screen, char joueur[4], int temps, elements *elem)
{
  packTempsInput *images = genererPackTempsInput(screen->buffer, elem, temps); //on génère les images nécessaires
  ...
  while(continuer) //on lance la boucle du menu
  {
    ...
    {
      redraw = false;
      afficherTempsInput(screen, images, elem, boutonOk, joueur); //on affiche l'image de tempsInput à nouveau
    }
  }
...
void afficherTempsInput(gameScreen *screen, packTempsInput *images, elements *elem, int boutonOk, char joueur[4])
{
