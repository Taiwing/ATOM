...
void partieSprint(gameScreen *screen, int niveauChoisi, gameConfig *config, packSons *sons, elements *elem)
{
  //on génère les images de la partie
  packPartie *images = genererPackPartie(elem, SPRINT); //on crée la structure d'où on va tirer les images de la partie
  ...
  ALLEGRO_TIMER *timer_partieSprint = al_create_timer(1.0 / FPS); //on règle le timer à un soixantième de seconde
  al_install_mouse();

  //on crée la file d'évènements
  ALLEGRO_EVENT_QUEUE *event_queue_partieSprint = al_create_event_queue();
  al_register_event_source(event_queue_partieSprint, al_get_display_event_source(screen->display)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  al_register_event_source(event_queue_partieSprint, al_get_timer_event_source(timer_partieSprint)); //on lie la file au timer
  al_register_event_source(event_queue_partieSprint, al_get_mouse_event_source()); //puis à la souris
  ...
  int timerSprint = 0; //va compter le nombre de 60ème secondes écoulées depuis le début de la partie

  //on lance le compte à rebours avant de commencer la partie
  troisDeuxUn(screen, images, elem, 0, 0, 0, 0);

  al_register_event_source(event_queue_partieSprint, al_get_keyboard_event_source()); //puis on prend en compte le clavier

  //on démarre le timer
  al_start_timer(timer_partieSprint);

  //boucle principale de la partie
  while(data->continuer)
  {
    ALLEGRO_EVENT ev; //on crée la variable qui va enregistrer les évènements
    al_wait_for_event(event_queue_partieSprint, &ev); //on attend qu'un évènement se produise

    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      //gestion du timerSprint
      timerSprint++;
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        else if(data->lignes >= 40) //ou s'il a gagné
        {
          data->continuer = false;
          data->victoire = true; //alors on indique à la boucle que le joueur a gagné
        }
        ...
        gameInput(data, screen, elem, images, event_queue_partieSprint, timer_partieSprint, sons, config); //gestion des input
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieSprint)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieSprint(screen, images, elem, data, timerSprint); //on affiche la partie à nouveau
      jouerSon(config->sound, sons, data->gameEvent, false); //va jouer ou arrêter le son du jeu
    }
  }

  if(data->victoire) //si le joueur a gagné
    highTemps(screen, timerSprint, elem); //on va sauvegarder les meilleurs temps comme pour les scores de marathon

  //on libère la mémoire
  destroyPackPartie(images);
  destroyGameData(data);
  al_destroy_timer(timer_partieSprint);
  al_unregister_event_source(event_queue_partieSprint, al_get_display_event_source(screen->display)); //devrait éviter l'erreur de SEGMENTATION
  al_destroy_event_queue(event_queue_partieSprint);
}

void afficherPartieSprint(gameScreen *screen, packPartie *images, elements *elem, gameData *data, int timerSprint)
{
  ...
  int minutes = (timerSprint/3600), secondes = ((timerSprint % 3600)/60), dixiemes = ((timerSprint % 60)/6);

  if(secondes < 1 && minutes == 0) //s'il ne s'est pas encore écoulé une seconde
  {
