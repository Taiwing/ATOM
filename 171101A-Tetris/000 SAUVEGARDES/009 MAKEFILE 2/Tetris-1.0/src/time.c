...
void highTemps(gameScreen *screen, int temps)
{
  ...
  if(place) //si le nouveau temps est supérieur à l'un de ceux du top (si place != 0)
  {
    ...
    tempsInput(screen, joueur, temps); //le joueur rentre son nom (et le panneau de temps est affiché)
...
{
  elementsFondTempsInput *elem = genererElementsFondTempsInput(); //on génére les éléments du fond de tempsInput
  ...
  destroyElementsFondTempsInput(elem);
