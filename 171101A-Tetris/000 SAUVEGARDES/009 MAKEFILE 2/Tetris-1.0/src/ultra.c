...
void partieUltra(gameScreen *screen, int niveauChoisi, gameConfig *config, packSons *sons)
{
  ...
  packPartie *images = genererPackPartie(elem, ULTRA); //on crée la structure d'où on va tirer les images de la partie
  ...
  ALLEGRO_TIMER *timer_partieUltra = al_create_timer(1.0 / FPS); //on règle le timer à un soixantième de seconde
  al_install_mouse();

  //on crée la file d'évènements
  ALLEGRO_EVENT_QUEUE *event_queue_partieUltra = al_create_event_queue();
  al_register_event_source(event_queue_partieUltra, al_get_display_event_source(screen->display)); //on lie la file d'évènements à la fenêtre (pour la fermer)
  al_register_event_source(event_queue_partieUltra, al_get_timer_event_source(timer_partieUltra)); //on lie la file au timer
  al_register_event_source(event_queue_partieUltra, al_get_mouse_event_source()); //puis à la souris

  //VARIABLES DU JEU:
  gameData *data = initialiserGameData(niveauChoisi); //on initialise la structure de données du jeu
  speed(data); //on établie la vitesse du jeu en fonction du niveau choisi
  int timerUltra = 10800; //va compter à rebours le nombre de 60ème secondes écoulées depuis le début de la partie

  //on lance le compte à rebours avant de commencer la partie
  troisDeuxUn(screen, images, elem, 0, 0, 0, 0);

  al_register_event_source(event_queue_partieUltra, al_get_keyboard_event_source()); //puis on prend en compte le clavier

  //on démarre le timer
  al_start_timer(timer_partieUltra);

  //boucle principale de la partie
  while(data->continuer)
  {
    ALLEGRO_EVENT ev; //on crée la variable qui va enregistrer les évènements
    al_wait_for_event(event_queue_partieUltra, &ev); //on attend qu'un évènement se produise

    if(ev.type == ALLEGRO_EVENT_TIMER) //quand 1/60s s'est écoulé
    {
      //gestion du timerUltra
      timerUltra--;
      ...
      else //sinon, on exécute la boucle normalement
      {
        ...
        else if(timerUltra <= 0) //ou s'il a gagné (le joueur est arrivé au bout du temps imparti)
        {
        ...
        gameInput(data, screen, elem, images, event_queue_partieUltra, timer_partieUltra, sons, config); //gestion des input
    ...
    if(data->redraw && al_is_event_queue_empty(event_queue_partieUltra)) //si redraw est vrai et que la file d'évènements est vide
    {
      ...
      data->redraw = false;
      afficherPartieUltra(screen, images, elem, data, timerUltra); //on affiche la partie à nouveau
      jouerSon(config->sound, sons, data->gameEvent, false); //va jouer ou arrêter le son du jeu
    }
  }

  if(data->victoire) //si le joueur a gagné (s'il a survécu jusqu'au bout du timer)
    highLignes(screen, data->lignes); //on teste le nombre de lignes pour voir s'i lest dans le top 5

  //on libère la mémoire
  destroyElementsFondPartie(elem);
  destroyPackPartie(images);
  destroyGameData(data);
  al_destroy_timer(timer_partieUltra);
  al_unregister_event_source(event_queue_partieUltra, al_get_display_event_source(screen->display)); //devrait éviter l'erreur de SEGMENTATION
  al_destroy_event_queue(event_queue_partieUltra);
}

void afficherPartieUltra(gameScreen *screen, packPartie *images, elementsFondPartie *elem, gameData *data, int timerUltra)
{
  ...
  int minutes = (timerUltra/3600), secondes = ((timerUltra % 3600)/60), dixiemes = ((timerUltra % 60)/6);

  if(timerUltra < 0) //si le timer est négatif
  {
    sprintf(strTimer, "0", dixiemes); //on affiche 0
  }
  else if(secondes < 1 && minutes == 0 && timerUltra >= 0) //s'il ne s'est pas encore écoulé une seconde
  {
  ...
  //affiche le timer juste en dessous
  al_draw_text(font, al_map_rgb(0, 0, 0), rect->x + rect->t + 247, rect->y2 + 9, ALLEGRO_ALIGN_RIGHT, strTimer);
