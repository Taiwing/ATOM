...
packPartie *genererPackPartie(elements *elem, int modeDeJeu, float scale)
{
  ...
  //puis on génère les blocs du jeu
  images->blocJaune = al_create_bitmap(ceil(BLOC_SIZE * scale), ceil(BLOC_SIZE * scale));
  al_set_target_bitmap(images->blocJaune);
  drawBlock(JAUNE, 0, 0, scale);
  images->blocCyan = al_create_bitmap(ceil(BLOC_SIZE * scale), ceil(BLOC_SIZE * scale));
  al_set_target_bitmap(images->blocCyan);
  drawBlock(CYAN, 0, 0, scale);
  images->blocMauve = al_create_bitmap(ceil(BLOC_SIZE * scale), ceil(BLOC_SIZE * scale));
  al_set_target_bitmap(images->blocMauve);
  drawBlock(MAUVE, 0, 0, scale);
  images->blocViolet = al_create_bitmap(ceil(BLOC_SIZE * scale), ceil(BLOC_SIZE * scale));
  al_set_target_bitmap(images->blocViolet);
  drawBlock(VIOLET, 0, 0, scale);
  images->blocOrange = al_create_bitmap(ceil(BLOC_SIZE * scale), ceil(BLOC_SIZE * scale));
  al_set_target_bitmap(images->blocOrange);
  drawBlock(ORANGE, 0, 0, scale);
  images->blocVert = al_create_bitmap(ceil(BLOC_SIZE * scale), ceil(BLOC_SIZE * scale));
  al_set_target_bitmap(images->blocVert);
  drawBlock(VERT, 0, 0, scale);
  images->blocRouge = al_create_bitmap(ceil(BLOC_SIZE * scale), ceil(BLOC_SIZE * scale));
  al_set_target_bitmap(images->blocRouge);
  drawBlock(ROUGE, 0, 0, scale);
  images->barreGrilleVerticale = al_create_bitmap(ceil(3 * scale), ceil(28 * scale));
  al_set_target_bitmap(images->barreGrilleVerticale);
  drawVerBarre(0, 0, scale);
  images->barreGrilleHorizontale = al_create_bitmap(ceil(28 * scale), ceil(3 * scale));
  al_set_target_bitmap(images->barreGrilleHorizontale);
  drawHorBarre(0, 0, scale);
...
packOptions *genererPackOptions(elements *elem, ALLEGRO_BITMAP *fondPrecedent, int keyMap[16][3], float scale)
{
  ...
  images->inputResWOff = genererBoutonOptions(IRESW, OFF, elem, keyMap, scale);
  images->inputResWOn = genererBoutonOptions(IRESW, ON, elem, keyMap, scale);
  images->inputResHOff = genererBoutonOptions(IRESH, OFF, elem, keyMap, scale);
  images->inputResHOn = genererBoutonOptions(IRESH, ON, elem, keyMap, scale);
  ...
  images->boutonResMinOff = genererBoutonOptions(BRESMIN, OFF, elem, keyMap, scale);
  images->boutonResMinOn = genererBoutonOptions(BRESMIN, ON, elem, keyMap, scale);
  images->boutonResMaxOff = genererBoutonOptions(BRESMAX, OFF, elem, keyMap, scale);
  images->boutonResMaxOn = genererBoutonOptions(BRESMAX, ON, elem, keyMap, scale);
  images->boutonAppliquerOff = genererBoutonOptions(APPLIQUER, OFF, elem, keyMap, scale);
  images->boutonAppliquerOn = genererBoutonOptions(APPLIQUER, ON, elem, keyMap, scale);
...
void destroyPackOptions(packOptions *images)
{
  ...
  al_destroy_bitmap(images->inputResWOff);
  al_destroy_bitmap(images->inputResWOn);
  al_destroy_bitmap(images->inputResHOff);
  al_destroy_bitmap(images->inputResHOn);
  ...
  al_destroy_bitmap(images->boutonAppliquerOff);
  al_destroy_bitmap(images->boutonAppliquerOn);
...
ALLEGRO_BITMAP *genererFondOptions(elements *elem, ALLEGRO_BITMAP *fondPrecedent, int recharge, float scale)
{
  ...
  al_draw_text(font, bleuActif, (elem->rectOptions->w / 2) + (elem->rectOptions->x), rect->y2 + (5 * scale), ALLEGRO_ALIGN_CENTRE, "RESOLUTION");
...
ALLEGRO_BITMAP *genererBoutonOptions(int typeDeBouton, int onOff, elements *elem, int keyMap[16][3], float scale)
{
  ...
  switch(typeDeBouton)
  {
    ...
    case BRESMIN:
      ...
      al_clear_to_color(couleurDeFond); //on remplie par la couleur déterminée au-dessus
      font = al_load_ttf_font("media_files/Raleway-Bold.ttf", (15 *scale), 0); //on charge la police "Raleway-Bold"
      al_draw_text(font, bleuActif, ((rect->x2 - rect->x - (rect->t * 2))/2), 0, ALLEGRO_ALIGN_CENTRE, "MIN");
      break;
    case BRESMAX:
      rect = elem->rectBOptionsResMax;
      //on crée la surface intérieure du bouton
      ...
      al_clear_to_color(couleurDeFond); //on remplie par la couleur déterminée au-dessus
      font = al_load_ttf_font("media_files/Raleway-Bold.ttf", (15 *scale), 0); //on charge la police "Raleway-Bold"
      al_draw_text(font, bleuActif, ((rect->x2 - rect->x - (rect->t * 2))/2), 0, ALLEGRO_ALIGN_CENTRE, "MAX");
      break;
    case APPLIQUER:
      rect = elem->rectBOptionsAppliquer;
      //on crée la surface intérieure du bouton
      ...
      al_clear_to_color(couleurDeFond); //on remplie par la couleur déterminée au-dessus
      font = al_load_ttf_font("media_files/Raleway-Bold.ttf", (15 *scale), 0); //on charge la police "Raleway-Bold"
      al_draw_text(font, bleuActif, ((rect->x2 - rect->x - (rect->t * 2))/2), 0, ALLEGRO_ALIGN_CENTRE, "APPLIQUER");
      break;
    case RESET:
      rect = elem->rectBOptionsReset;
      //on crée la surface intérieure du bouton
...
void genererFondDujeu(ALLEGRO_BITMAP *fond)
{
  ...
  for(; y <= al_get_bitmap_height(fond); y+=8) //on a enlevé le premier terme car il était inutile, on laisse un point virgule quand même
  {
    al_draw_line(x, y, x2, y2, bleuLignes, 1.5);
    x2+=4;
  }
  for(; x2 <= al_get_bitmap_width(fond); x2+=4)
  {
    al_draw_line(x, y, x2, y2, bleuLignes, 1.5);
    x+=4;
  }
  for(; y2 <= al_get_bitmap_height(fond); y2+=8)
  {
    al_draw_line(x, y, x2, y2, bleuLignes, 1.5);
    x+=4;
  }
...
packImages *genererPackImages(elements *elem, int keyMap[16][3], float scale)
{
  ...
  return images; //on renvoit le pack d'images (comment ça a marché sans ça !?)
}
