...
void highLignes(gameScreen *screen, int lignes, packImages *images, elements *elem)
{
  ...
  if(place) //si le nouveau lignes est supérieur à l'un de ceux du top (si place != 0)
  {
    ...
    lignesInput(screen, joueur, lignes, images, elem); //le joueur rentre son nom (et le panneau de lignes est affiché)
...
{
  FILE *lignes = fopen("game_files/lignes.lin", "r");
...
void nouveauLignes(char joueur[4], int lignes, int place)
{
  ...
  FILE *fLignes = fopen("game_files/lignes.lin", "w+"); //on lit et écrit dans le fichier en effaçant son contenu au préalable
...
void lignesInput(gameScreen *screen, char joueur[4], int lignes, packImages *images, elements *elem)
{
  regenererPackImages(images, elem, PISTLINPUT, ULTRA, screen->buffer, NULL, lignes, screen->scaleBuff); //on régénère le packSTLInput pour avoir la bonne image
  ...
  while(continuer) //on lance la boucle du menu
  {
    ...
    {
      redraw = false;
      afficherLignesInput(screen, images->imagesSTLInput, elem, boutonOk, joueur); //on affiche l'image de lignesInput à nouveau
    }
  }
...
void afficherLignesInput(gameScreen *screen, packSTLInput *images, elements *elem, int boutonOk, char joueur[4])
{
  targetBuffer(screen);
...
void chargerListeElemFondLignesInput(gameScreen *screen, elements *elem, Liste *lElem)
{
